...
@Deprecated(
    message = "Use forEachWithIterator inline lambda instead",
    replaceWith = ReplaceWith("this.forEachWithIterator(action)"),
    level = DeprecationLevel.WARNING
)
@Suppress("Deprecation")
public tailrec suspend fun <T> Node<T>.forEachSuspend(
    action: suspend (T) -> Unit
): Unit = when(this) {
    Node.Nil -> Unit
    is Node.Cons<T> -> {
        action(head)
        tail.forEachSuspend(action)
    }
}
...
@Deprecated(
    message = "Use loopOnWithIterator inline lambda instead",
    replaceWith = ReplaceWith("this.loopOnWithIterator(action)"),
    level = DeprecationLevel.WARNING
)
@Suppress("Deprecation")
public suspend inline fun <reified U : T, T> Node<T>.loopOnSuspend(
    crossinline action: suspend (U) -> Unit
): Unit = forEachSuspend {
    if (it is U) action(it)
}
